import ExperienceCard from "./Eductaions";
import { MobileEducation } from "./MobileEducation";
import { RxPeople } from "react-icons/rx";
import { SerEdu } from "./SerEdu";
import { FaTrophy } from "react-icons/fa";
import { FaGraduationCap } from "react-icons/fa";
import { BiSolidSchool } from "react-icons/bi";

export const AboutEduk = ({ display = true }) => {
  return (
    <section className="container flex items-center justify-center pt-5 pb-25">
      <div className="w-[85%] flex items-center flex-col justify-center">
        {display && (
          <SerEdu
            btn_text={"LIFE TIME"}
            heading={"Education & Experience"}
            para={
              "We craft digital, graphic and dimensional thinking, to create category leading brand experiences that have meaning ."
            }
          />
        )}
        <div className=" w-[70%] hidden  md:flex pt-25  flex-col gap-10">
          <ExperienceCard
            date={"2019-22"}
            border={"border-red-200"}
            icon={<FaTrophy className={`text-red-400 text-4xl`}/>}
            iconBg={"bg-red-100"}
            circleBg={"bg-red-400"}
            institute={"GHI Themes - Web & App IT Company"}
          />
          <ExperienceCard
            date={"2014-18"}
            border={"border-green-200"}
            rotate={"rotate-180"}
            icon={<RxPeople className={`text-green-400 text-4xl`}/>}
            iconBg={"bg-green-100"}
            circleBg={"bg-green-400"}
            institute={"ABC themes - Web Design IT Company"}
          />
          <ExperienceCard
            date={"2012-13"}
            border={"border-yellow-200"}
            iconsColor={""}
            icon={<FaGraduationCap className={`text-yellow-400 text-4xl`}/>}
            iconBg={"bg-yellow-100"}
            circleBg={"bg-yellow-400"}
            institute={"MSc IT Master Degree"}
          />
          <ExperienceCard
            date={"2017-08"}
            border={"border-blue-200"}
            rotate={"rotate-180"}
            iconsColor={"text-blue-400"}
            icon={<BiSolidSchool className={`text-blue-400 text-4xl`}/>}
            iconBg={"bg-blue-100"}
            circleBg={"bg-blue-400"}
            institute={"High / Higher secondary school"}
          />
        </div>
        <div className="w-full mt-20 pt-25 flex md:hidden flex-col gap-35">
          <MobileEducation
           date={"2017-08"}
           border={"border-blue-200"}
           circleBg={"bg-blue-400"}
           institute={"High / Higher secondary school"}
           />
          <MobileEducation
            date={"2014-18"}
            border={"border-green-200"}
            circleBg={"bg-green-400"}
            institute={"ABC themes - Web Design IT Company"}
           />
            <MobileEducation
            date={"2012-13"}
            border={"border-yellow-200"}
            circleBg={"bg-yellow-400"}
            institute={"MSc IT Master Degree"}
          />
          <MobileEducation
            date={"2017-08"}
            border={"border-blue-200"}
            circleBg={"bg-blue-400"}
            institute={"High / Higher secondary school"}
          />
        </div>
      </div>
    </section>
  );
};
